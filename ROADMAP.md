...
- ✅ **LANZADA en la App Store (2026-08-05)** — versión 1.1 **aprobada** ("Ready for Distribution") + las **2 suscripciones IAP aprobadas** (Monthly/Annual). Live en Australia + resto del mundo. App ID **6787267468** · https://apps.apple.com/app/settlia/id6787267468
  - **Trampa post-aprobación:** la app salía "no disponible en tu región" porque estaba **Removed from Sale** (banner en la versión → "Pricing and Availability → add it back"). Se re-activó la disponibilidad → propaga en ~24h (la búsqueda de la App Store indexa lo último; la página directa aparece antes).
  - **UE pendiente:** los 27 países de la UE requieren **Trader Status (DSA, Digital Services Act)**. Ya enviado, estado **"In Review"** por Apple (Compliance → Digital Services Act). Al verificarse, la UE se activa sola (~24h). Ojo: los datos de contacto del trader (nombre/dirección/email/teléfono) se muestran **públicos** en la ficha EU. Tax forms (ABN/GST, W-8BEN) todos Active.
- **Saga de rechazos superada (2026-07/08) — histórico:**
  1. **2.1(a)** (login) → path email+password oculto (`Login.tsx` `pwMode` + `signInPassword`) para el demo account `demo.review@settlia.app`.
  2. **3.1.2** (link EULA en la metadata) → bloque de suscripción + link al EULA estándar de Apple + Privacy Policy en la App Description; License Agreement = Apple standard EULA.
  3. **2.3.10** (referencia a Android en el binario) → se quitó la mención a Android en el string `aiConsent.who` (en iOS toda la IA va por Groq).
  4. **3.1.2(c)** (el importe a cobrar no era lo más prominente + el free trial de 7 días) → paywall nativo rediseñado: **precio grande y dominante** + línea subordinada "1 semana gratis · luego $6.99/mes" (trial expuesto desde RevenueCat `introPrice` en `iap.ts`). Se **conservó** el trial (Opción 2) mostrándolo bien.
