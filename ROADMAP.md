...
- ✅ **Multi-moneda en las pills** de balance global (2026-08): convierte el aporte de cada grupo a una **moneda principal** (perfil `profiles.main_currency`, `migrate_v11`, editable en AccountModal; default = la más usada entre los grupos activos) vía la tasa diaria (`fxCache`). Fallback sin convertir mientras cargan las tasas.
...
- ✅ **Android developer verification** (nuevo requisito de Google, deadline 30-sep-2026): package **`app.settlia.pwa`** + 3 claves de firma **REGISTRADAS** (17-jul-2026). Sin esto, apps Android se quitan de Play y dejan de instalarse (incluso sideload) en dispositivos certificados de ciertos países. Recordar registrar aquí cualquier package/clave nuevo.
...
- ✅ **Disponibilidad:** re-habilitada en Pricing & Availability (estaba "Removed from Sale"). ⚠️ **EU BLOQUEADA — falta el DSA "Trader Status".** En App Store Connect → Availability, todos los países UE/EEE salen con **"Trader Status Not Provided"** (X roja). NO está "in review": está **SIN declarar**. **Acción del usuario:** App Store Connect → cuenta/**Business** o el "View" junto al país → declarar **"I am a trader"** (Settlia tiene IAP → es trader) + datos de contacto (nombre/dirección/email/teléfono, se muestran públicos en la UE) → verificar el email/SMS de Apple → los países UE se habilitan en ~24h. Cambio de cuenta, no de código.
...
- ✅ **Permisos de borrado de miembros (2026-09):** solo el **dueño** puede quitar miembros (`GroupSettings`/`Members`/`UsersModal`). Antes cualquiera podía eliminar a cualquiera, incluido el dueño —que perdía el acceso al grupo (caso real `Espagne 2026`). ⬜ **Pendiente:** endurecerlo server-side (`update_group_data` no valida quién edita `members`).
- ⬜ Migrar `settlement.proof` base64 → Storage (con Fase 1).
- ⬜ **Constituir empresa australiana (Pty Ltd) — DIFERIDO, no ahora.** Se pesó incorporar en un país sin impuestos: descartado (la residencia fiscal sigue a dónde vives / se dirige la empresa; reglas CFC + economic substance lo anulan; más fricción con bancos/Apple/Stripe que ahorro). Camino correcto = **Pty Ltd en AU** cuando toque. Costos aprox (AUD): registro ASIC ~$597 (una vez), ABN/TFN gratis, business name ~$44/año; **ASIC annual review ~$329/año** + contador ~$800–2,000/año. Requiere **Director ID** (gratis) y director residente en AU. Servicios tipo EasyCompanies/Sleek ~$550–750 lo hacen todo. **Además lo necesitamos igual para el DSA Trader Status de Apple (piden ABN de comerciante).** Confirmar tasas en asic.gov.au (cambian cada julio). Orientativo, NO asesoría fiscal — validar con contador antes de decidir estructura.
- ⬜ **`last_seen_at` en `profiles`** para métricas de "activos que ABREN" (hoy el Admin mide activos por acción vía el log de actividad; no cuenta quien solo abre la app sin añadir nada). Marcar la fecha al autenticar/abrir en el cliente + `active_open_7d/30d` en `get_admin_stats`.
