...
- **Filtros en la sección de Gastos (`ExpenseList`):** botón **Filtros** (icono nuevo `filter`) junto al título con badge de filtros activos. Panel con chips de **persona** (paga o participa; muestra burbuja), **categoría** (solo categorías presentes) y **mes-año** (reutiliza `monthsWithExpenses`/`monthKey`/`monthLabel` de `report.ts`). Combinables, muestran "N de Total", botón **Limpiar**, y mensaje `filter.none` si nada coincide. Strings `filter.*` en `i18n.ts`.
...
- **Receipt/proof images:** currently payment proof is a **base64 data URL inside the group JSON** (`settlement.proof`) and scanned receipts are **discarded** after OCR. Planned: **Supabase Storage** (private bucket, path with `groupId`, signed URLs, member-only RLS) for both proofs and scanned receipts; migrate legacy base64. (ROADMAP Fase 1.)
- **Recurring expenses** generate a real expense per cycle via `processRecurring` (client-side, on group open, catch-up ≤12), but emit **no Activity/notification per occurrence** — only `recurring_added` on rule creation. Planned: log each generated occurrence; optional server cron generation. (ROADMAP Fase 1.)
