...
- `supabase/functions/parse-expense|scan-receipt|transcribe/` — Deno Edge Functions, all on **Groq** (OpenAI-compatible API, key `STT_API_KEY`). `parse-expense` (text→expense, `llama-3.1-8b-instant`), `scan-receipt` (vision, `meta-llama/llama-4-scout-17b-16e-instruct` — override with `AI_VISION_MODEL`, e.g. Maverick or a Claude model via OpenRouter), `transcribe` (STT, `whisper-large-v3-turbo`). All use `response_format: json_object` + **server-side sanitize**. **`transcribe` + `parse-expense` deployed; `scan-receipt` ready to deploy.**
