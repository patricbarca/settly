...
- `supabase/functions/parse-expense|scan-receipt|transcribe/` — Deno Edge Functions, all on **Groq** (OpenAI-compatible API, key `STT_API_KEY`). `parse-expense` (text→expense, `llama-3.1-8b-instant`), `scan-receipt` (vision, `meta-llama/llama-4-maverick-17b-128e-instruct` — Scout was too weak on real receipts; override with `AI_VISION_MODEL`. No `response_format` here: Groq vision returns `json_validate_failed` under strict JSON mode, so we rely on the prompt + `extractJson`), `transcribe` (STT, `whisper-large-v3-turbo`). All use `response_format: json_object` + **server-side sanitize**. **`transcribe` + `parse-expense` deployed; `scan-receipt` ready to deploy.**
