...
- `supabase/functions/parse-expense|scan-receipt|transcribe/` — Deno Edge Functions. `parse-expense` (text→expense) + `transcribe` (STT) use **Groq** (OpenAI-compatible API, key `STT_API_KEY`); `scan-receipt` (vision) uses Claude (`ANTHROPIC_API_KEY`). `parse-expense` defaults: `llama-3.1-8b-instant` via `https://api.groq.com/openai/v1/chat/completions` (override with `AI_TEXT_API_KEY`/`AI_TEXT_API_URL`/`AI_TEXT_MODEL`), `response_format: json_object`, and **server-side sanitize** (valid member IDs, allowed category, valid interval). **`transcribe` deployed (Groq `whisper-large-v3-turbo`); `parse-expense`/`scan-receipt` not deployed yet.**
...
- **Three AI paths wired** (Edge Functions, keys server-side only): `parse-expense` (text→expense+category, **Groq `llama-3.1-8b-instant`**), `scan-receipt` (vision, Claude), `transcribe` (Groq Whisper). Interpret uses the LLM when Pro/quota, else the local regex parser. **`parse-expense`/`scan-receipt` still need deploying** to actually work.
