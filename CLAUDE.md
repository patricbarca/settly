...
### Dual-currency group display toggle (Pro)
- **`Group.secondaryCurrency` / `Group.displayCurrency`** (JSON, no SQL): in `GroupSettings`, Pro users pick a second ISO currency for the group and a segmented toggle to switch which of the two is shown app-wide. Amounts are still **stored** in `group.currency` only — this is display-only.
- **`src/lib/fxCache.ts`** (`useDailyRate`/`getDailyRate`): caches the `from→to` rate in localStorage per day per pair (via `convertCurrency` from `fx.ts`/`convert-currency` Edge Function) so flipping the toggle doesn't re-hit the API every render.
- **`src/lib/displayCurrency.ts`** (`useGroupMoney(group)`): drop-in replacement for `money(amount, group.currency)` → `money(amount)` that auto-converts when the toggle is on Pro + secondary currency; falls back to the primary currency instantly (no flash of wrong numbers) while the day's rate loads. Wired into `Hero.tsx`, `Balances.tsx`, `ExpenseList.tsx` (both the expense list and the per-expense detail).
- **Deliberately NOT converted** (must stay in the group's real, stored currency): `PaySheet` (the actual pay-link amount), the underlying `tr.amount`/`s.amount` numbers passed into `MarkPaidModal`/`PaySheet` state in `Balances.tsx` (only their *display* text converts, not the value used for the real payment flow), and all data-entry forms (`ExpenseForm`, `AddExpense`, `ItemizedExpenseEditor`) — the toggle is display-only, entry/payment always happens in `group.currency`.
- **Not yet wired to the toggle** (still `group.currency`-only, follow-up if wanted): `ReportModal`/`report.ts` (PDF/CSV export), `Analytics.tsx`/`CategoryChart.tsx` (stats), `NotificationsBell.tsx`, `MarkPaidModal.tsx`'s own amount text.

### Currency conversion on scanned receipts (Pro) + native iOS OAuth fix
- **Scan-and-convert (Pro-gated):** `scan-receipt` already detected the receipt's ISO currency (e.g. scanning a ticket in Vietnam → `VND`) but discarded it. Now `ScanReceiptModal` compares it to `group.currency`: **Pro** → calls the new Edge Function `convert-currency` (exchangerate-api.com, secret `EXCHANGE_RATE_API_KEY`) once to get the day's rate, applies it to every item/fee/tax line, and stores `Expense.originalAmount/originalCurrency/fxRate` for a "Converted from X (rate)" badge in `ExpenseList`. **Free** → shows an upsell banner with a Paywall CTA instead of converting. `src/lib/fx.ts` (`convertCurrency`). The LLM is never used for the rate itself (would hallucinate/be stale) — only exchange rates from the dedicated API are trusted.
- **`convert-currency` deployed** (via `mcp__Supabase__deploy_edge_function`, unlike other functions which need manual dashboard paste). **Needs the `EXCHANGE_RATE_API_KEY` secret set manually** in Supabase Dashboard → Project Settings → Edge Functions → Secrets (no MCP tool sets secrets).
- **Native OAuth fix (Capacitor/iOS):** Google/Apple sign-in was completing in Safari instead of returning to the wrapped app. Fixed via a custom URL scheme `app.settlia.pwa://` (`Info.plist` `CFBundleURLTypes`), PKCE flow on the Supabase client, and on native platforms opening the OAuth URL via `@capacitor/browser` + an `appUrlOpen` listener that calls `exchangeCodeForSession` (`src/lib/auth.ts`). Web login unchanged. **Needs `app.settlia.pwa://auth/callback` added to Supabase Dashboard → Authentication → URL Configuration → Redirect URLs.**
