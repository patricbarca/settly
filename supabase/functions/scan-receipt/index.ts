...
- fees: extra charges added ON TOP of the items — surcharge, service charge, card/payment surcharge, delivery, weekend/public-holiday surcharge. Each {name, amount}. Do NOT put tax, tip, subtotal or total in fees.
  - DISCOUNTS / PROMOTIONS: a line that REDUCES the total (a promo, "multibuy"/"X for $Y" saving, member discount, coupon, a negative "-$2.00" amount, or a line between subtotal and total that lowers it) MUST be included in fees as a NEGATIVE amount, e.g. {"name":"Discount","amount":-2.00}. Never drop it — it is why total < subtotal. Keep the item at its full printed price and record the saving as this negative fee. Re-check: subtotal + (all fees, discounts negative) + (tax if not included) = total.
