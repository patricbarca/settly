...
//
// NOTA (bugs históricos, no reintroducir):
//   1. El builder de supabase-js v2 es "thenable" pero NO expone `.catch(...)`
//      — `.insert(x).catch(()=>{})` lanza un TypeError y tumba la función (500).
//      Usar siempre `await` + try/catch.
//   2. Un espacio/newline invisible pegado al copiar un secreto APNs (típico en
//      APNS_TEAM_ID) corrompe el JWT → APNs responde 403 InvalidProviderToken.
//      Por eso se hace `.trim()` defensivo a los secretos.
// ============================================================
...
async function sendApns(
  ...
  try {
    ...
    console.error("[apns] jwt sign failed — revisa APNS_KEY_P8:", e instanceof Error ? e.message : String(e));
    return 0;
  }
