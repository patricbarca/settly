...
const DICT: Record<string, { es: string; en: string }> = {
  ...
  "aiConsent.who": { es: "A quién: nuestros proveedores de IA (Groq para escaneo/voz/texto; Google en el dictado por voz de Android). No se usa para entrenar modelos ni se guarda tras procesarlo.", en: "Who it goes to: our AI providers (Groq for scan/voice/text; Google for Android voice dictation). It is not used to train models and is not stored after processing." },
  ...
  "pay.overpayWarn": { es: "Ojo: {who} ya solo debe {amt} en total. Este pago es mayor — puede que este gasto ya esté incluido en un pago a otra persona (modo Simplificado). Revisa antes de confirmar para no duplicarlo.", en: "Heads up: {who} only owes {amt} in total now. This payment is larger — this expense may already be covered by a payment to someone else (Simplified mode). Double-check before confirming to avoid duplicating it." },
  "pay.overpaySettled": { es: "Ojo: {who} ya no tiene deuda pendiente en el grupo. Es probable que esto ya se haya saldado (p. ej. dentro de otro pago). Registrarlo podría duplicar el dinero.", en: "Heads up: {who} has no pending balance left in the group. This was likely already settled (e.g. inside another payment). Recording it may duplicate money." },
